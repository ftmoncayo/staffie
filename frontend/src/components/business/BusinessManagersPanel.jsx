...
function BusinessManagersPanel({ businessId }) {
  ...
  async function handleAdd(item) {
    setError('')
    try {
      await api.addBusinessManager(businessId, item.id)
      await refresh()
    } catch (err) {
  ...
  async function handleRemove(userId) {
    ...
    try {
      await api.removeBusinessManager(businessId, userId)
      await refresh()
    } catch (err) {
      setError(err.message)
    } finally {
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-6">
      ...
      <p className="mt-1 text-sm text-text-faint">
        Managers can edit this business's details. Only admins can assign or remove managers.
      </p>
...
export default BusinessManagersPanel
